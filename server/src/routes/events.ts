...
const shape = z.object({
  ...
  courtesy: z.number().int().nonnegative().optional(),
...
function scope(req: AuthRequest) {
  return globalAdmin(req.auth!.role) ? undefined : req.auth!.producerId ?? -1
}

eventsRouter.get('/', async (req: AuthRequest, res) => {
  const requested = req.query.producerId ? Number(req.query.producerId) : undefined
  const producerId = globalAdmin(req.auth!.role) ? requested : scope(req)
...
eventsRouter.get('/code/:code', async (req: AuthRequest, res) => {
...
eventsRouter.post('/', async (req: AuthRequest, res) => {
  const p = shape.parse(req.body)
  const producerId = globalAdmin(req.auth!.role) ? p.producerId : req.auth!.producerId
  if (!producerId) return res.status(400).json({ message: 'Produtora obrigatória.' })
  const created = await prisma.event.create({ data: { ...p, producerId } })
  await audit(req, req.auth!.id, producerId, 'create', 'event', String(created.id))
...
eventsRouter.put('/:id', async (req: AuthRequest, res) => {
  ...
  if (!existing) return res.status(404).json({ message: 'Evento não encontrado.' })
  if (!globalAdmin(req.auth!.role) && existing.producerId !== req.auth!.producerId) {
    return res.status(403).json({ message: 'Acesso negado a evento de outra produtora.' })
  }
  const p = shape.partial().parse(req.body)
  const updated = await prisma.event.update({ where: { id }, data: p })
  await audit(req, req.auth!.id, existing.producerId, 'update', 'event', String(id))
  res.json(updated)
})
