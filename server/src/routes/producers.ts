...
producersRouter.post('/',requireRoles('admin-master','admin'),async(req:AuthRequest,res)=>{const p=z.object({name:z.string().min(2),document:z.string().min(5)}).parse(req.body);const created=await prisma.producer.create({data:p as any});await audit(req,req.auth!.id,created.id,'create','producer',String(created.id));res.status(201).json(created)})
...
producersRouter.get('/:id/events',async(req:AuthRequest,res)=>{const producerId=Number(req.params.id);if(!Number.isFinite(producerId))return res.status(400).json({message:'Produtor inválido.'});if(!globalAdmin(req.auth!.role)&&req.auth!.producerId!==producerId)return res.status(403).json({message:'Acesso negado a eventos de outra produtora.'});const events=await prisma.event.findMany({where:{producerId},orderBy:{id:'desc'}});res.json({ok:true,events})})
