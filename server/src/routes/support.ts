...
supportRouter.post('/tickets',async(req:AuthRequest,res)=>{
 ...
 const producerId=tenantProducerId(req,req.body.producerId); if(!producerId)return res.status(400).json({message:'Selecione uma produtora.'})
...
supportRouter.patch('/tickets/:id',async(req:AuthRequest,res)=>{
 const id=Number(req.params.id), where=tenantWhere(req,req.body.producerId)
...
supportRouter.post('/tickets/:id/messages',async(req:AuthRequest,res)=>{
 ...
 const where=tenantWhere(req,req.body.producerId); const ticket=await prisma.serviceTicket.findFirst({where:{id,...where}}); if(!ticket)return res.status(404).json({message:'Chamado não encontrado.'})
...
supportRouter.get('/summary',async(req:AuthRequest,res)=>{
 const where=tenantWhere(req,req.query.producerId), rows=await prisma.serviceTicket.findMany({where})
...
supportRouter.get('/sla-policies',async(req:AuthRequest,res)=>{const where=tenantWhere(req,req.query.producerId);res.json(await prisma.slaPolicy.findMany({where,orderBy:{priority:'asc'}}))})
supportRouter.get('/integrations',async(req:AuthRequest,res)=>{const where=tenantWhere(req,req.query.producerId);res.json(await prisma.supportIntegration.findMany({where,orderBy:{name:'asc'}}))})
