...
supportRouter.post('/tickets',async(req:AuthRequest,res)=>{
 ...
 const producerId=tenantProducerId(req.auth!,req.body.producerId); if(!producerId)return res.status(400).json({message:'Selecione uma produtora.'})
...
supportRouter.post('/tickets/:id/messages',async(req:AuthRequest,res)=>{
 ...
 const where=tenantWhere(req.auth!,req.body.producerId); const ticket=await prisma.serviceTicket.findFirst({where:{id,...where}}); if(!ticket)return res.status(404).json({message:'Chamado não encontrado.'})
 const msg=await prisma.ticketMessage.create({data:{...parsed.data,ticketId:id,producerId:ticket.producerId} as any}); res.status(201).json(msg)
})
...
supportRouter.get('/sla-policies',async(req:AuthRequest,res)=>{const where=tenantWhere(req.auth!,req.query.producerId);res.json(await prisma.slaPolicy.findMany({where,orderBy:{priority:'asc'}}))})
supportRouter.get('/integrations',async(req:AuthRequest,res)=>{const where=tenantWhere(req.auth!,req.query.producerId);res.json(await prisma.supportIntegration.findMany({where,orderBy:{name:'asc'}}))})
