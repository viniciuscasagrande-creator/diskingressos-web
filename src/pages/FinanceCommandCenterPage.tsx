...
export default function FinanceCommandCenterPage({events,producerId,notify,onNavigate}:Props){
 ...
 {error&&<div className="finance-command-warning"><AlertTriangle size={16}/>{error}</div>}
  <section className="finance-command-kpis">
    <Kpi icon={WalletCards} label="Saldo disponível" value={brl(summary?.availableBalanceCents)} sub="Disponível para operação e repasse" tone="blue" onClick={()=>onNavigate('finance')}/>
    <Kpi icon={Clock3} label="Saldo futuro" value={brl(summary?.futureBalanceCents)} sub="Recebíveis e liquidações previstas" tone="green" onClick={()=>onNavigate('finance-receivables')}/>
    <Kpi icon={BanknoteArrowUp} label="A pagar" value={brl(summary?.payablesCents)} sub="Obrigações financeiras em aberto" tone="orange" onClick={()=>onNavigate('finance-payables')}/>
    <Kpi icon={HandCoins} label="Repasses pendentes" value={brl(summary?.pendingPayoutsCents)} sub={`${summary?.pendingPayoutsCount||0} solicitação(ões)`} tone="purple" onClick={()=>onNavigate('finance-payouts')}/>
    <Kpi icon={Percent} label="Margem média Spread" value={`${((summary?.avgMarginBps||0)/100).toFixed(2)}%`} sub={`${summary?.spreadSimulations||0} simulações persistidas`} tone="cyan" onClick={()=>onNavigate('finance-spread')}/>
    <Kpi icon={AlertTriangle} label="Divergências" value={String(summary?.divergences??0)} sub="Itens que exigem conciliação" tone="red" onClick={()=>onNavigate('finance-reconciliation')}/>
  </section>
  <section className="finance-command-health">
    <div onClick={()=>onNavigate('finance-gateways')} role="button" tabIndex={0} style={{cursor:'pointer'}}><Activity size={18}/><span><b>{summary?.activeGateways||0}</b> gateways ativos</span></div>
    <div onClick={()=>onNavigate('finance-operators')} role="button" tabIndex={0} style={{cursor:'pointer'}}><ShieldCheck size={18}/><span><b>{summary?.activeAcquirers||0}</b> adquirentes ativas</span></div>
    <div onClick={()=>onNavigate('finance-methods')} role="button" tabIndex={0} style={{cursor:'pointer'}}><CreditCard size={18}/><span><b>{summary?.methods||0}</b> métodos configurados</span></div>
    <div onClick={()=>onNavigate('finance-refunds')} role="button" tabIndex={0} style={{cursor:'pointer'}}><Undo2 size={18}/><span><b>{brl(summary?.refundsCents)}</b> em estornos</span></div>
    <div onClick={()=>onNavigate('finance-receivables')} role="button" tabIndex={0} style={{cursor:'pointer'}}><CircleDollarSign size={18}/><span><b>{brl(summary?.receivablesCents)}</b> em recebíveis</span></div>
  </section>
...
function Kpi({icon:Icon,label,value,sub,tone,onClick}:{icon:any;label:string;value:string;sub:string;tone:Tone;onClick?:()=>void}){return <article className={`finance-command-kpi ${tone}`} onClick={onClick} style={{cursor:onClick?'pointer':'default'}} role={onClick?'button':undefined} tabIndex={onClick?0:undefined}><span className="icon"><Icon size={20}/></span><div><small>{label}</small><strong>{value}</strong><span>{sub}</span></div></article>}
