...
export default function FinanceCommandCenterPage({events,producerId,notify,onNavigate}:Props){
 ...
 <section className="finance-command-kpis"><Kpi icon={WalletCards} label="Saldo disponível" value={brl(summary?.availableBalanceCents)} sub="Disponível para operação e repasse" tone="blue"/><Kpi icon={Clock3} label="Saldo futuro" value={brl(summary?.futureBalanceCents)} sub="Recebíveis e liquidações previstas" tone="green"/><Kpi icon={BanknoteArrowUp} label="A pagar" value={brl(summary?.payablesCents)} sub="Obrigações financeiras em aberto" tone="orange"/><Kpi icon={HandCoins} label="Repasses pendentes" value={brl(summary?.pendingPayoutsCents)} sub={`${summary?.pendingPayoutsCount||0} solicitação(ões)`} tone="purple"/><Kpi icon={Percent} label="Margem média Spread" value={`${((summary?.avgMarginBps||0)/100).toFixed(2)}%`} sub={`${summary?.spreadSimulations||0} simulações persistidas`} tone="cyan"/><Kpi icon={AlertTriangle} label="Divergências" value={String(summary?.divergences??0)} sub="Itens que exigem conciliação" tone="red"/></section>
 <section className="finance-command-health"><div><Activity size={18}/><span><b>{summary?.activeGateways||0}</b> gateways ativos</span></div><div><ShieldCheck size={18}/><span><b>{summary?.activeAcquirers||0}</b> adquirentes ativas</span></div><div><CreditCard size={18}/><span><b>{summary?.methods||0}</b> métodos configurados</span></div><div><Undo2 size={18}/><span><b>{brl(summary?.refundsCents)}</b> em estornos</span></div><div><CircleDollarSign size={18}/><span><b>{brl(summary?.receivablesCents)}</b> em recebíveis</span></div></section>
