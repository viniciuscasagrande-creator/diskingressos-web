  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="bg-white border border-[#E2E8F0] rounded-card p-1.5 shadow-xs flex items-center gap-1 overflow-x-auto">
        ...
        <button
          onClick={() => setActiveTab('rmk-payments')}
          className={`flex items-center gap-2 px-3.5 py-2 rounded-btn text-xs font-bold transition-all shrink-0 ${
            activeTab === 'rmk-payments'
              ? 'bg-rose-600 text-white shadow-xs'
              : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'
          }`}
        >
          <Clock3 size={15} />
          <span>Pagamentos Pendentes (PIX)</span>
        </button>
        ...
        <div className="space-y-6">
          <div className="bg-white p-5 rounded-card border border-[#E2E8F0] shadow-xs">
            ...
            <h2 className="text-[20px] font-extrabold text-[#0E1726] tracking-tight">
              Central de Remarketing & Recuperação de Oportunidades
            </h2>
      ...
      {/* 3. Carts View (Real SQLite DB) */}
      {activeTab === 'rmk-carts' && (
        <RecoveryCenterPage producerId={producerId} events={events} mode="carts" notify={notify} />
      )}

      {/* 4. Pending Payments View (PIX) */}
      {activeTab === 'rmk-payments' && (
        <RecoveryCenterPage producerId={producerId} events={events} mode="payments" notify={notify} />
      )}

      {/* 5. Inactive Clients */}
      {activeTab === 'rmk-inactive' && (
        <RecoveryCenterPage producerId={producerId} events={events} mode="inactive" notify={notify} />
      )}

      {/* 6. Post Event */}
      {activeTab === 'rmk-postevent' && (
        <RecoveryCenterPage producerId={producerId} events={events} mode="postevent" notify={notify} />
      )}
      ...
      {/* 8. WhatsApp Remarketing */}
      {activeTab === 'rmk-whatsapp' && (
