...
export default function EventCard({
  ...
  const isSelect = selectionMode || isComparing
  const isSel = selected || isSelectedForCompare
  ...
  return (
    <article
      className={`event-card event-card-clickable ${isSelect ? 'compare-selectable' : ''} ${isSel ? 'compare-selected' : ''}`}
      ...
      role="button"
      tabIndex={0}
      onClick={() => (isSelect ? handleSelect(event) : onOpen(event))}
      onKeyDown={e => {
        if (e.key === 'Enter' || e.key === ' ') {
          e.preventDefault()
          isSelect ? handleSelect(event) : onOpen(event)
        }
      }}
    >
      {isSelect && (
        <div
          className="compare-check event-compare-checkbox-wrap"
          data-testid="event-compare-checkbox"
          aria-hidden="true"
          onClick={e => {
            e.stopPropagation()
            handleSelect(event)
